/* 
 * Copyright (c) 2025, sparkmon <dev@sparkmon.com>
 * All rights reserved.
 */
#define _GNU_SOURCE
#include <stdio.h>
...
int main(int argc, char **argv)
{
    ...
    struct drm_xe_sync sync = {
        ...
        .flags          = DRM_XE_SYNC_FLAG_SIGNAL, /* out-fence */
        .handle         = sync_handle,
        .timeline_value = 0,
    };

    /* 7) Bind BO into the VM at BIND_ADDRESS (synchronous bind) */
    ...
    /* 8) Pick a RENDER engine instance */
    ...
    /* 9) Create exec queue for that engine + VM */
    ...
    printf("Batch submitted, waiting for completion...\n");

    /* 11) Wait on syncobj signaled by EXEC */
    wait_syncobj(fd, sync_handle);
    printf("Render batch completed.\n");
